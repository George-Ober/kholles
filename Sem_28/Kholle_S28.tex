...
\begin{question_kholle}[{Soit $n_{0} \in \N$ et $f:[n_{0}, +\infty[ \to \R$ une fonction continue, décroissante et minorée par $m \in \R$.
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      % Axes
      \draw[->] (-0.5,0) -- (7.5,0) node[right] {$x$}; % x-axis
      \draw[->] (0,-0.5) -- (0,4.5) node[above] {$f(x) = \frac{1}{x}$}; % y-axis
      
      % Define the function f(x) = 1/x from x=1 to x=6, scaling the y-axis
      \draw[domain=1:7,samples=100,smooth,thick,blue] plot (\x, {1/\x*4}) node[right] {};
      
      % Draw integer points on x-axis and their corresponding values of f(n)
      \foreach \n in {1,2,...,6} {
          % Mark x = n on x-axis
          \draw[dashed] (\n, 0) -- (\n, {4/\n});
          % Place a dot at (n, f(n))
          \filldraw (\n, {4/\n}) circle (2pt) node[above right] {$f(\n)$};
          
          % Draw the rectangle from (n,0) to (n+1, f(n)) for each interval [n, n+1]
          \draw[red, thick] (\n, {4/\n}) -- (\n+1, {4/\n}) -- (\n+1, {4/(\n + 1)});
          
          % Fill the area not covered by the integral as the difference (shaded in gray)
          ...
          \draw[red] (9, {4/\n}) -- (10, {4/\n}) -- (10, {4/(\n+1)});
          % Label the integral area
          % \node[blue] at (\n+0.5, 0.25) {$\int_{\n}^{\n+1} f(x) \, dx$};
      }
      
      \draw[dashed] (9, 4) -- (9, 0) -- (10, 0) -- (10, 1);
      
      % \draw (8.9, -0.1) rectangle (10.1, 4.1);
      % \fill[gray] (10, 3) -- (8, 3) -- (9, 1.5);
