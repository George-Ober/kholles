...
\begin{document}
	...
	\begin{abstract}
		Bienvenue très chers camarades sereins, ce document contient les questions de khôlles de mathématiques de la MP1 de Fermat. Il est coécrit par Kylian Boyet, Hugo \textit{Vangi}lluwen (qui maintient la structure du projet, la compilation et le paquet kholles.sty) avec la contribution de George Ober et de Jérémie Menard.
...
\pagebreak\section{Semaine 22}
	...
	\begin{question_kholle}[{Soient $(E,F)$ deux $\mathbb{K}$-espaces vectoriels $f \in \mathcal{L}(E, F)$, $\mathcal{F}=(x_{i})_{i\in I}$ une base de $E$.
		...
		Alors $$\text{Vect} \underbrace{ f(\mathcal{F}) }_{ \left\{ x_{i} \mid i \in I \right\}  } = f(\text{Vect}\mathcal{ F})$$}]
